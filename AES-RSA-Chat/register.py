...
if re.match(pattern, login) != None:
    if login in userlist:
        ...
        if password == passwordconf:
            pattern = re.compile(
                r'^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[^\w\s]).{8,257}$')
            if re.match(pattern, password) != None:
                print('Default access level : {}. You can change it manually for this account in file users.json'.format(
                    DEFAULT_ACCESS_LEVEL))
                register_time = str(datetime.datetime.now())
                obj[login] = [str((SHA256.new(password.encode(
                    'utf-8'))).hexdigest()), register_time, 'No data', 0, 0, 0]
