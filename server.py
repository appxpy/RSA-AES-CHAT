...
class ClientThread(Thread):
	...
	def run(self):
		try:
			...
			publickeyclipem = self.conn.recv(2048) ########## 02 ##########
			...
			data = self.conn.recv(2048) ########## 03 ##########
			data = json.loads(decrypt(data, self.publickeycli, self.privatekeysrvpem))
			...
			if self.login in CLIENTS_USERS:
					...
					if self.ip in BLOCKED_LIST:
						...
						if BLOCKED_LIST[self.ip] != 'Inf':
							...
							if (datetime.datetime.now() - datetime.datetime.strptime(BLOCKED_LIST[self.ip], '%Y-%m-%d %H:%M:%S.%f')).total_seconds() < 60:
								...
								timestamp = 60 - (datetime.datetime.now() - datetime.datetime.strptime(BLOCKED_LIST[self.ip], '%Y-%m-%d %H:%M:%S.%f')).total_seconds()
						...
						if BLOCKED_LIST[self.login] != 'Inf':
							...
							if (datetime.datetime.now() - datetime.datetime.strptime(BLOCKED_LIST[self.login], '%Y-%m-%d %H:%M:%S.%f')).total_seconds() < 60:
								...
								timestamp = 60 - (datetime.datetime.now() - datetime.datetime.strptime(BLOCKED_LIST[self.login], '%Y-%m-%d %H:%M:%S.%f')).total_seconds()
				...
				if self.login in USERS.keys():
					...
					if USERS[data['login']][0] == data['password']:
						...
						broadcast(message , self.conn, self.publickeycli, self.privatekeysrvpem)

						CLIENTS_USERS[self.login] = [self.conn, datetime.datetime.now()]
		...
		while not self.stopped(): 
			...
			try:
				...
				if message != b'': 
					...
					message = decrypt(message, self.publickeycli, self.privatekeysrvpem)

					print(f'[{datetime.datetime.now()}] [{self.ip}:{self.port}] [{self.name}] {self.login} > {message}')
					
					message_to_send = "< {} > {}".format(self.login, message).encode('utf-8')
					MESSAGES[str(datetime.datetime.now())] = message_to_send.decode("utf-8")

					broadcast(message_to_send, self.conn, self.publickeycli, self.privatekeysrvpem) 
...
def kick(connection, sit):
	...
	if sit == 'kick':
		...
		connection.send(encrypt('You have been kicked from the server.'.encode('utf-8'), thread.publickeycli, thread.privatekeysrvpem))

	else:
		
		connection.send(encrypt('Your account have been permanently blocked.'.encode('utf-8'), thread.publickeycli, thread.privatekeysrvpem))
	...
	while True:
		...
		if cmd.lower() == 'stop':
			...
			if len(data) != 2:
				...
				print('[{}] [Server] [Main] > Error while parsing command args. Usage : unban <user>'.format(datetime.datetime.now())) 
				...
				if data in USERS.keys():
					...
					if data in BLOCKED_LIST.keys():
						...
						print('[{}] [Server] [Main] > User {} succesefully unbanned.'.format(datetime.datetime.now(), data))
			...
			if len(data) != 2:
				...
				print('[{}] [Server] [Main] > Error while parsing command args. Usage : kick <user>'.format(datetime.datetime.now())) 
			...
			if len(data) == 1:
				...
				print('[{}] [Server] [Main] > Error while parsing command args. Usage : broadcast <message>'.format(datetime.datetime.now())) 
			...
			for item in BLOCKED_LIST.keys():
				...
				if BLOCKED_LIST[item] != 'Inf':
					...
					left_time = round((timestamp - datetime.datetime.strptime(BLOCKED_LIST[item], '%Y-%m-%d %H:%M:%S.%f')).total_seconds(), 1)
					...
					if left_time > 60:
						...
						status = 'TEMPORARY BLOCKED FOR {}'.format(60 - left_time)
...
if __name__ == "__main__":
	...
	try:
		...
		handshakeProcess = Thread(target=handshakeThread, args=[server, publickeysrvpem, privatekeysrvpem])
